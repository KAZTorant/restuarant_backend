...
<i>Analizdə görmək istədiyiniz məlumatı yazın...</i>"""
    ...
    async def handle_user_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            logger.info("=== AI ANALYSIS FLOW START ===")

            # Step 1: Analyze with Gemini
            logger.info("STEP 1: Sending request to Gemini for analysis")
            analysis_result = await GeminiService.analyze_user_request(user_text)
            logger.info(f"Gemini analysis result: {analysis_result}")

            instructions = analysis_result.get('instructions', '')
            query_spec = analysis_result.get('query', {})

            logger.info(f"Instructions: {instructions}")
            logger.info(f"Query spec: {query_spec}")

            if not instructions or not query_spec:
                logger.warning(
                    "Missing instructions or query_spec from Gemini")
            ...
            logger.info(
                f"Query execution success: {query_results.get('success')}")

            if not query_results.get('success'):
                logger.error(
                    f"Query execution failed: {query_results.get('error')}")
                await processing_msg.edit_text(
            ...
            logger.info("STEP 3: Serializing data and generating analysis")
            await processing_msg.edit_text(
                "🤖 <b>ANALİZ EDİLİR...</b>\n\n"
                "✅ Sorğu anlaşıldı\n"
                "✅ Məlumatlar toplandı\n"
            ...
            serialized_data = AnalyticsSerializer.serialize_for_analysis(
                query_results)
            logger.info(
                f"Data serialization completed: {serialized_data.get('success', False)}")

            analysis_report = await GeminiService.generate_analysis(serialized_data, instructions)
            logger.info(
                f"Analysis report generated, length: {len(analysis_report)} chars")
            ...
            logger.info("=== AI ANALYSIS FLOW COMPLETED SUCCESSFULLY ===")
