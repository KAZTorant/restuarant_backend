...
class TransferOrderItemsAPIView(APIView):
    ...
    def post(self, request, table_id):
        ...
        try:
            ...
            return Response({"error": "Source/target table, order, or meal not found."},
        ...
        try:
            with transaction.atomic():
                transferred = self._bulk_transfer(items_qs, tgt_order)
    ...
    @staticmethod
    def _bulk_transfer(items_qs, new_order):
        """
        ...
        OrderItem.objects.filter(pk__in=pks).update(order=new_order)
        return OrderItem.objects.filter(pk__in=pks)
